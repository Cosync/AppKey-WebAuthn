...
```
    await appKeyAuth.auth.signupConfirm({handle:'user_email@appkey.io', atttestationObject}) 
```
...
```
    do {
        let signupData = await appKeyAuth.auth.signupConfirm({handle:'user_email@appkey.io', atttestationObject});
...
The *loginComplete()* function is the second step in registering a user with an AppKey application, called after *login()*. It takes the the user’s handle (email or phone) and an Assertion object to verify the login. 
